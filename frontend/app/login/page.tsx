...
export default function LoginPage() {
  ...
  const validateForm = () => {
    ...
    if (password.length < 1) { newErrors.password = 'Password cannot be empty'; isValid = false; }
    setErrors(newErrors);
    return isValid;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err?.response?.data?.message || 'Login failed. Is the backend running?');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <div className="w-full max-w-md rounded-2xl p-8 border shadow-xl bg-white dark:bg-zinc-900 border-slate-100 dark:border-zinc-800">
        <div className="text-center mb-8">
          ...
          <p className="text-xs mt-1 text-slate-500 dark:text-zinc-400">Sign in to check your resume's ATS score.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="block text-xs font-bold uppercase text-slate-700 dark:text-zinc-300">Password</label>
              <Link href="/forgot-password" className="text-xs text-indigo-600 hover:underline font-medium">Forgot?</Link>
            </div>
        ...
        <p className="text-center text-xs mt-6 text-slate-600 dark:text-zinc-400">
          ...
          <Link href="/signup" className="text-indigo-600 font-semibold hover:underline">Register Free</Link>
        </p>
      </div>
    </div>
  );
